...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ isOpen, onClose, cart, clearCart, onOrderComplete }) => {
  ...
  const finalizeOrder = async (ref: string) => {
    ...
    const vendorsToNotify = Array.from(new Set<Vendor>(cart.map(item => item.vendor)))
      .filter((v: Vendor): v is Vendor => !!v && !!v.email)
      .map((v: Vendor) => ({ name: v.name, email: v.email! }));
    ...
    try {
      ...
      }).catch(err => console.warn("[Notification] Could not reach backend.", err));
    } catch (e) {
        ...
        <div className="p-6 md:p-8 flex-1 overflow-y-auto">
          {step === Step.FORM && (
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 p-4 rounded-sm border border-gray-100 space-y-2 font-sans">
              ...
              <form onSubmit={handlePay} className="space-y-5">
                <div className="grid grid-cols-2 gap-4 font-sans">
                  <div>
                     ...
                     <input required type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} className="w-full border border-gray-300 px-3 py-3 text-sm focus:border-black focus:outline-none rounded-none" />
                  </div>
                  <div>
                     <label className="block text-xs uppercase tracking-widest text-gray-500 mb-1.5">Email</label>
                     <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full border border-gray-300 px-3 py-3 text-sm focus:border-black focus:outline-none rounded-none" />
                  </div>
                </div>
                ...
                {deliveryMethod === 'Delivery' ? (
                  <div className="space-y-4 font-sans">
                     <div>
                       ...
                       <select value={selectedZone.id} onChange={(e) => { const zone = DELIVERY_ZONES.find(z => z.id === e.target.value); if (zone) setSelectedZone(zone); }} className="w-full border border-gray-300 px-3 py-3 text-sm focus:border-black focus:outline-none rounded-none bg-white">
                         {DELIVERY_ZONES.map(zone => (
                           <option key={zone.id} value={zone.id}>{zone.name} - {CURRENCY} {zone.price}</option>
                         ))}
                       </select>
                     </div>
                     <div>
                       <label className="block text-xs uppercase tracking-widest text-gray-500 mb-1.5">Address & Suggestions</label>
                       <textarea required value={address} onChange={(e) => setAddress(e.target.value)} rows={2} className="w-full border border-gray-300 px-3 py-3 text-sm focus:border-black focus:outline-none rounded-none" placeholder="House Number, Street Name, or click a landmark below..." />
                         ...
                         <div className="mt-2">
                           ...
                           <div className="flex flex-wrap gap-2">
                             {ZONE_LANDMARKS[selectedZone.id].map(landmark => (
                               <button key={landmark} type="button" onClick={() => addLandmark(landmark)} className="flex items-center text-[10px] bg-gray-100 hover:bg-gray-200 text-gray-600 px-2 py-1 rounded transition-colors">
                  ...
                  <div className="bg-yellow-50 border border-yellow-100 p-4 rounded-sm font-sans">
                ...
                <div className="pt-4 border-t border-gray-100">
                  <h3 className="font-serif text-lg mb-4 flex items-center"><Smartphone className="mr-2 h-5 w-5" /> Mobile Money Payment</h3>
                  <div className="grid grid-cols-3 gap-2 mb-4 font-sans">
                    {(['Airtel', 'MTN', 'Zamtel'] as Provider[]).map((net) => (
                      <button key={net} type="button" onClick={() => setNetwork(net)} className={`py-3 text-xs font-bold border transition-colors ${network === net ? 'bg-black text-white border-black' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-400'}`}>{net}</button>
                    ))}
                  </div>
                  <div className="font-sans">
                    ...
                    <div className="flex">
                      ...
                      <input type="tel" required value={phone} onChange={(e) => setPhone(e.target.value)} className="block w-full border border-gray-300 px-3 py-3 text-sm focus:border-black focus:outline-none rounded-none" placeholder="97 1234567" />
                    </div>
                  </div>
                </div>

                <div className="pt-2 font-sans">
                  <button type="submit" disabled={isSubmitting} className={`w-full py-4 font-bold text-sm uppercase tracking-widest transition-all flex items-center justify-center space-x-2 ${isSubmitting ? 'bg-stone-800 text-stone-400 cursor-not-allowed' : 'bg-green-700 hover:bg-green-800 text-white'}`}>
                  ...
                  <p className="text-[10px] text-center text-gray-400 mt-3 uppercase tracking-wider">Secure Payment • Delivery by Partner</p>
                </div>
              </form>
            </div>
          )}
          ...
          {step === Step.INITIATING && (
            <div className="flex flex-col items-center justify-center py-16 text-center space-y-6 animate-fade-in font-sans">
          ...
          {step === Step.PENDING_USSD && (
            <div className="flex flex-col items-center justify-center py-8 text-center space-y-6 animate-fade-in font-sans">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm max-w-xs mx-auto">Payment prompt sent to <strong>+260 {phone}</strong></p>
                <div className="bg-yellow-50 border border-yellow-100 p-3 mt-4 text-xs text-yellow-800 rounded">Enter PIN on mobile to authorize {CURRENCY} {total.toFixed(2)}</div>
              </div>
            </div>
          )}
          ...
          {step === Step.SUCCESS && (
            <div className="flex flex-col items-center justify-center py-12 text-center space-y-6 animate-scale-in font-sans">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm max-w-xs mx-auto mb-2">Thank you for supporting Zambian artisans.</p>
                {deliveryMethod === 'Collection' ? (
                ...
                <div className="bg-gray-50 p-3 mb-6 text-[10px] text-gray-500 font-mono">TRANS_REF: {transactionRef}</div>
                <button onClick={onClose} className="px-8 py-3 bg-black text-white text-xs uppercase tracking-widest hover:bg-gray-800 font-sans font-bold">Continue Shopping</button>
              </div>
            </div>
          )}
        </div>
